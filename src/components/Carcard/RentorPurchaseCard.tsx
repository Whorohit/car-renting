...
const RentorPurchaseCard: React.FC<Props> = ({ data = {}, payment = {} }) => {
    ...
    return (
     ...
     border-neutral-300 border-2 p-2  w-full  md:w-[90%]  justify-between rounded-lg h-[rem]  tex ' onClick={() => {
            ...
            <div className=' basis-2/3 '>
                ...
                <h1 className=' mt-2  text-xs md:text-base  text-neutral-600 '>
                ...
                <h1 className='text-black font-semibold  text-sm md:text-base  text-left mt-2 '>On {formatDate(payment.createdAt)} </h1>
                ...
                <div className='flex justify-center flex-col items-start py-2'>
                 {data.RentorSell==="Rent" &&  <h1 className='font-semibold    text-sm md:text-base' > Total Price:  <span className='font-normal '> Rs{payment.price
                    }</span></h1>}
                    <h1 className='font-semibold    text-sm md:text-base space-x-3' > OrderID:  <span className=' font-normal '> {payment.razorpay_order_id}</span></h1>
                    <h1 className='font-semibold   text-sm  md:text-base' > PaymentID  <span className='font-normal '> {payment.razorpay_payment_id
                    }</span></h1>
                </div>
